...
class RDFExtResultSink(val resultDir: String) extends Sink {
  ...
  override def consume(item: Item, model: Model, modName:String): Unit = {
    ...
    this.synchronized {
      ...
      RDFDataMgr.write(fos, prepareModel(model.getNsPrefixMap, item, modName), Lang.TTL)
      fos.close()
    }

    val externalResultFile = targetFile.parent/ s"${item.shaSum}-external.ttl"

    this.synchronized {
      val fos = new FileOutputStream(externalResultFile.toJava, false)
      RDFDataMgr.write(fos, model, Lang.TTL)
      fos.close()
    }
  }

  def prepareModel(prefixMap:java.util.Map[String,String], item:Item, modName:String)={
    ...
    val additionalPrefixMap: Map[String, String] = Map(
    ...
    model.setNsPrefixes(additionalPrefixMap.asJava)
    ...
    model.add(
      ResourceFactory.createStatement(
        ResourceFactory.createResource(s"${model.getNsPrefixMap.get("myMod")}${item.shaSum}.ttl#this"),
        ResourceFactory.createProperty(s"${model.getNsPrefixMap.get("mod")}generatedResult"),
        ResourceFactory.createResource(s"${model.getNsPrefixMap.get("myMod")}${item.shaSum}-external.ttl#this")))
  }
}
